...
class GalleryUmkmPlaceModel extends Model
{
    protected $DBGroup          = 'default';
    protected $table            = 'gallery_umkm_place';
    protected $primaryKey       = 'id';
    protected $returnType       = 'array';
    protected $allowedFields    = ['id', 'umkm_place_id', 'url'];
    ...
    public function get_gallery_api($umkm_place_id = null) {
        $query = $this->db->table($this->table)
            ->select('url')
            ->where('umkm_place_id', $umkm_place_id)
            ->get();
        return $query;
    }
    ...
    public function update_gallery_api($id = null, $data = null) {
        $queryDel = $this->db->table($this->table)->delete(['umkm_place_id' => $id]);
        $queryIns = $this->add_gallery_api($id, $data);
        return $queryDel && $queryIns;
    }

    public function delete_gallery_api($id = null) {
        return $this->db->table($this->table)->delete(['umkm_place_id' => $id]);
    }
}
